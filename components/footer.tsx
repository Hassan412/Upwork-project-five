      ...
      <div className="w-full flex flex-col md:px-8 px-4 lg:px-36">
        <div className="flex justify-center items-center gap-4 pt-8">
          {FooterData.Links.map((link, index) => (
            <Link key={index} className="lg:text-lg md:text-base text-sm" href={link.href}>
              {link.label}
            </Link>
          ))}
        </div>
        <div className="flex flex-wrap flex-1 gap-8 justify-center h-40 border-b-[1px] py-12 border-green-500">
          {FooterData.Cards.map((card: string, index) => (
            <div key={index} className="relative h-[40px] w-[65px]">
              <Image
                ...
                src={card}
                fill
                className="object-contain grayscale rounded-md"
              />
            </div>
          ))}
        </div>
